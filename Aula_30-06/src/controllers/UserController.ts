...
export class UserController {
    ...
    async list(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            // next(error) jogo o erro pro errorHandler, que decide o status e a mensagem certa dependendo do erro
            next(error)
        }
    }

    // GET /users/:id -> buscar um cupinxa pelo id
    ...
    async create(req: Request, res: Response, next: NextFunction) {
        try {
            ...
            const { name, email, password } = req.body // pega name, email, password pelo corpo da requisição
